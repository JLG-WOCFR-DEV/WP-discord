...
- **Gouvernance & secrets** : les tokens sont désormais chiffrés, horodatés à chaque rotation et signalés au-delà de 90 jours, mais restent mutualisés et sans segmentation par profil — migrer vers une architecture multi-tenant dédiée avec rotation automatisée et délégation fine.【F:discord-bot-jlg/inc/class-discord-admin.php†L465-L517】【F:discord-bot-jlg/inc/class-discord-admin.php†L892-L996】【F:discord-bot-jlg/inc/helpers.php†L233-L300】
...
- **Contrôle d’accès monolithique** : les pages d’administration et endpoints REST reposaient sur `manage_options` (ou une clé statique) sans différencier les profils. La v3 introduit désormais des capacités dédiées (`manage_discord_bot`, `manage_discord_profiles`, `view_discord_analytics`, `export_discord_analytics`) automatiquement attribuées aux administrateurs, ouvrant la voie à une gouvernance affinée (délégation par rôle, scopes par profil, audit trail).【F:discord-bot-jlg/inc/class-discord-admin.php†L36-L116】【F:discord-bot-jlg/inc/class-discord-rest.php†L200-L337】
...
| Gouvernance & secrets | Chiffrement AES/HMAC, migration automatique et horodatage des rotations avec alertes au-delà de 90 jours, mais stockage mutualisé sans audit trail complet.【F:discord-bot-jlg/inc/class-discord-admin.php†L465-L517】【F:discord-bot-jlg/inc/class-discord-admin.php†L892-L996】【F:discord-bot-jlg/inc/helpers.php†L233-L340】 | Coffres par locataire, rotation planifiée, historiques d’accès et délégation fine. | Isoler les secrets par profil (table dédiée), déclencher des rotations assistées et journaliser les accès individuels. |
...
| Accès & permissions | Capacités dédiées (`manage_discord_bot`, `manage_discord_profiles`, `view_discord_analytics`, `export_discord_analytics`) désormais disponibles pour déléguer l’administration sans passer par `manage_options`, mais les scopes fins (par profil, par action) restent à généraliser.【F:discord-bot-jlg/inc/class-discord-admin.php†L36-L116】【F:discord-bot-jlg/inc/class-discord-rest.php†L200-L337】 | Rôles modulaires, scopes d’API par serveur, audit trail complet. | Étendre la matrice d’autorisations par profil, proposer des clés API périmétrées et historiser les opérations sensibles. |
...
6. **Segmenter la gouvernance** : capitaliser sur les nouvelles capacités (`manage_discord_bot`, `manage_discord_profiles`, `view_discord_analytics`, `export_discord_analytics`) pour bâtir une matrice d’autorisations complète (clés API périmétrées, audit trail) et déléguer la gestion des profils conformément aux exigences SOC2/ISO.【F:discord-bot-jlg/inc/class-discord-admin.php†L36-L116】【F:discord-bot-jlg/inc/class-discord-rest.php†L200-L337】

### Nouvelles priorités alignées sur les standards pro (2024)

1. **Mettre en place la rotation planifiée des secrets** : capitaliser sur l’horodatage/alertes existants pour automatiser les rappels, stocker les tokens par profil dans une table sécurisée et déléguer la rotation par locataire.【F:discord-bot-jlg/inc/class-discord-admin.php†L465-L517】【F:discord-bot-jlg/inc/class-discord-admin.php†L892-L996】【F:discord-bot-jlg/inc/helpers.php†L233-L340】
...
4. **Segmenter l’accès administratif** : exploiter les capacités dédiées (`manage_discord_bot`, `manage_discord_profiles`, `view_discord_analytics`, `export_discord_analytics`) pour générer des clés API limitées par profil et historiser les actions sensibles via l’Event Logger (SOC2/ISO).【F:discord-bot-jlg/inc/class-discord-admin.php†L36-L116】【F:discord-bot-jlg/inc/class-discord-rest.php†L200-L337】
